...
public class Solution {
			...
			while (quitProgram == false) {
				...
				if (choice.equalsIgnoreCase("Q")) {
					...
					try {
						...
						Long valueToCalculate = Long.valueOf(choice);

						// negative value specified, throw an exception and
						// notify user.
						if (valueToCalculate.longValue() < 0 || choice.contains(".")) {
							throw new NumberFormatException();
						}

						String result = calculateSolution(valueToCalculate.longValue());
						System.out.println(result);
						System.out.println("");
					} catch (NumberFormatException nfe) {
						System.out
								.println("Invalid Or Blank long Value Provided! "
										+ choice);
						System.out
	...
	public static String calculateSolution(long population) {

		Long calculatedPopulation = new Long(0); // current calculated population (used to compute extra plates)
		long digits = 0; // digits in solution
		long letters = 0; // letters in solution
